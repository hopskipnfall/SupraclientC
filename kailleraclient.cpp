...
DLLEXP kailleraSetInfos(kailleraInfos *infosp) {
  ...
  AppendMenu(favoritelistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  AppendMenu(favoritelistMenu, MF_STRING | MF_POPUP, (UINT)favoritelistSubMenu,
             "Copy");
  ...
  AppendMenu(recentlistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  AppendMenu(recentlistMenu, MF_STRING | MF_POPUP, (UINT)recentlistSubMenu,
             "Copy");
  ...
  AppendMenu(waitinglistMenu, MF_STRING | MF_POPUP, 0xFFE7,
             "Refresh Waiting Games");
  AppendMenu(waitinglistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  AppendMenu(waitinglistMenu, MF_STRING | MF_POPUP, 0xFFE8, "Connect");
  AppendMenu(waitinglistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  AppendMenu(waitinglistMenu, MF_STRING | MF_POPUP, (UINT)waitinglistSubMenu,
             "Copy");
  ...
  AppendMenu(serverlistMenu, MF_STRING | MF_POPUP, 0xFFEF,
             "Refresh Server List");
  AppendMenu(serverlistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  ...
  AppendMenu(serverlistMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);
  AppendMenu(serverlistMenu, MF_STRING | MF_POPUP, (UINT)serverlistSubMenu,
             "Copy");
  ...
  AppendMenu(gameUserlistMenu, MF_STRING | MF_POPUP, (UINT)gameUserlistSubMenu,
             "Mute Player");
  ...
  AppendMenu(txtChatroomMenu, MF_STRING | MF_POPUP, 0xDDD6,
             "Get My IP Address");
  AppendMenu(txtChatroomMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);

  AppendMenu(txtChatroomMenu, MF_STRING | MF_POPUP, (UINT)txtChatroomPmSubMenu,
             "Private Messages");
  AppendMenu(txtChatroomPmSubMenu, MF_STRING, 0xDDD4, "Off");
  AppendMenu(txtChatroomPmSubMenu, MF_STRING, 0xDDD5, "On");
  AppendMenu(txtChatroomMenu, MF_SEPARATOR | MF_POPUP, NULL, NULL);

  AppendMenu(txtChatroomMenu, MF_STRING | MF_POPUP, (UINT)txtChatroomSubMenu,
             "Stealth Mode");
  ...
  // PM
  AppendMenu(userlistMenu, MF_STRING | MF_POPUP, (UINT)0xB2DD,
             "Private Message");
  ...
  AppendMenu(userlistMenu, MF_STRING | MF_POPUP, (UINT)userlistSubMenu,
             "Silence");
...
DLLEXP kailleraSelectServerDialog(HWND parent) {
  ...
  hDefaultFont = (HFONT)CreateFont(
      -11, 0, 0, 0, FW_REGULAR, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0,
      "Microsoft Sans Serif"); // GetStockObject(DEFAULT_GUI_FONT);
  mainHwnd = parent;
  ...
  // Create Child Window
  form1 = CreateWindow(
      "Supraclient", "SupraclientCPPE https://god-weapon.github.io",
      formProperties, xPos, yPos, 800, 600, NULL, NULL, hInstance, NULL);

  kailleraInit();
  createChatroom();
  createInitialWindow();

  Serverlist3DAdditem("Right Click to get EmuLinker Server List.", NULL, NULL,
                      NULL, NULL, NULL, NULL);
  kServerlistAdditem("Right Click to get Kaillera Server List.", NULL, NULL,
                     NULL, NULL, NULL, NULL);
  waitinglistAdditem("Right Click to get EmuLinker Waiting Games.", NULL, NULL,
                     NULL, NULL, NULL, NULL);
...
DLLEXP kailleraModifyPlayValues(void *values, int size) {
  ...
  // After Initial Delay Setup
    ...
    while (frameCount < connectionType) {
      ...
      if (sizeOfEinput == -1) {
        ...
        sizeOfEinput = -1;
        MessageBox(form1, "Game Ended!  Didn't receive a response for 15s.",
                   "Game Ended!", NULL);
        return sizeOfEinput;
      }
      DoEvents();
  ...
  if (returnInputSize == true) {
    ...
    // ex). MAME32k is 2 bytes of input/frame.  If there are 4 players, return 8
    // bytes
...
long CALLBACK SubProcTxtMaxUsers(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_KEYDOWN: {
    ...
    case VK_RETURN: {
      // Max Users
      GetWindowText(txtMaxUsers, maxUsersG,
                    GetWindowTextLength(txtMaxUsers) + 1);
  ...
  case WM_COMMAND: {
    ...
    case BN_CLICKED: {
      if (hwndCtl == btnLogin) {
        ...
        c.flags = LVFI_STRING;
        int pos =
            SendMessage(lstRecentList, LVM_FINDITEM, (WPARAM)-1, (LPARAM)&c);
        ...
        if (pos > -1) {
          ...
          int pos = SendMessage(lstFavoriteList, LVM_FINDITEM, (WPARAM)-1,
                                (LPARAM)&c);
 ...
 sendto(p2pSocket,
  ...
  case WM_NOTIFY: {
    ...
    if ((pnmh->hwndFrom == lstUserlist) && (pnmh->code == LVN_ITEMACTIVATE)) {
      ...
      popupMenu(2);
      return 0;
    } else if ((pnmh->hwndFrom == lstUserlist) &&
               (pnmh->code == LVN_COLUMNCLICK)) {
    ...
    // lstGameUserlist
    if ((pnmh->hwndFrom == lstGameUserlist) &&
        (pnmh->code == LVN_ITEMACTIVATE) && imOwner) {
      // kickRequest();
      return 0;
    } else if ((pnmh->hwndFrom == lstGameUserlist) &&
               (pnmh->code == NM_RCLICK)) {
      popupMenu(4);
      return 0;
    }
    ...
    if ((pnmh->hwndFrom == lstGamelist) && (pnmh->code == LVN_ITEMACTIVATE)) {
      if (myGameID == -1 &&
          SendMessage(chkJoinDbl, BM_GETCHECK, 0, 0) == BST_CHECKED)
        joinGameRequest();
      return 0;
    } else if ((pnmh->hwndFrom == lstGamelist) && (pnmh->code == NM_RCLICK)) {
      popupMenu(3);
      return 0;
    } else if ((pnmh->hwndFrom == lstGamelist) &&
               (pnmh->code == LVN_COLUMNCLICK)) {
      // The user clicked on one of the columns.  Here we will save
      // the index of the subitem that the column refers to.  Then
      // call ListView_SortItemsEx() to sort the list items.
    ...
    // lstServerListK
    if ((pnmh->hwndFrom == lstServerListK) &&
      ...
      return 0;
    } else if ((pnmh->hwndFrom == lstServerListK) &&
               (pnmh->code == NM_RCLICK)) {
      popupMenu(5);
      return 0;
    } else if ((pnmh->hwndFrom == lstServerListK) &&
    ...
    // lstServerList3D
    if ((pnmh->hwndFrom == lstServerList3D) &&
        (pnmh->code == LVN_ITEMACTIVATE)) {
      userQuitRequest();
      // Get Currently Selected item
      ...
      recentlistAdditem(server, ip, location);
      return 0;
    } else if ((pnmh->hwndFrom == lstServerList3D) &&
               (pnmh->code == NM_RCLICK)) {
      popupMenu(5);
      return 0;
    } else if ((pnmh->hwndFrom == lstServerList3D) &&
    ...
    // lstWaitingList
    if ((pnmh->hwndFrom == lstWaitingList) &&
        (pnmh->code == LVN_ITEMACTIVATE)) {
      userQuitRequest();
      // Get Currently Selected item
      ...
      recentlistAdditem(server, ip, location);
      return 0;
    } else if ((pnmh->hwndFrom == lstWaitingList) &&
               (pnmh->code == NM_RCLICK)) {
      popupMenu(6);
      return 0;
    } else if ((pnmh->hwndFrom == lstWaitingList) &&
    ...
    // lstFavoriteList
    if ((pnmh->hwndFrom == lstFavoriteList) &&
        (pnmh->code == LVN_ITEMACTIVATE)) {
      userQuitRequest();
      ...
      recentlistAdditem(server, ip, location);
    } else if ((pnmh->hwndFrom == lstFavoriteList) &&
               (pnmh->code == NM_RCLICK)) {
      popupMenu(8);
      return 0;
    } else if ((pnmh->hwndFrom == lstFavoriteList) &&
    ...
    if ((pnmh->hwndFrom == lstRecentList) && (pnmh->code == LVN_ITEMACTIVATE)) {
      ...
      popupMenu(7);
      return 0;
    } else if ((pnmh->hwndFrom == lstRecentList) &&
               (pnmh->code == LVN_COLUMNCLICK)) {
      // The user clicked on one of the columns.  Here we will save
      // the index of the subitem that the column refers to.  Then
      // call ListView_SortItemsEx() to sort the list items.
  ...
  case SC_SUPRARECVWAITING: {
    ...
    case FD_READ: {
      bytesRecvWaiting =
          recv(mySocketWaiting, myBuffWaiting, sizeof(myBuffWaiting), NULL);
      parseWaitingGames();
      return 0;
    }
    }
  ...
  case SC_SUPRARECV3D: {
    ...
    case FD_READ: {
      bytesRecvServerList3D = recv(mySocket3D, myBuffServerList3D,
                                   sizeof(myBuffServerList3D), NULL);
      parseServerList3D();
      return 0;
    }
    }
  ...
  case SC_SUPRARECVK: {
    ...
    case FD_READ: {
      bytesRecvServerListK =
          recv(mySocketK, myBuffServerListK, sizeof(myBuffServerListK), NULL);
      parseServerListK();
      return 0;
    }
    }
...
void createInitialWindow() {
  ...
  // Create Favoritelist
  lstFavoriteList = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                   listviewProperties, 10, 30, 770, 440, form1,
                                   NULL, hInstance, NULL);
  SendMessage(lstFavoriteList, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  SendMessage(lstFavoriteList, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Waitinglist
  lstWaitingList = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                  listviewProperties, 10, 30, 770, 440, form1,
                                  NULL, hInstance, NULL);
  SendMessage(lstWaitingList, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  ...
  SendMessage(lstWaitingList, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Recentlist
  lstRecentList = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                 listviewProperties, 10, 30, 770, 440, form1,
                                 NULL, hInstance, NULL);
  SendMessage(lstRecentList, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  LvCol.pszText = "IP Address\0";
  ...
  SendMessage(lstRecentList, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Serverlist
  lstServerListK = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                  listviewProperties, 10, 30, 770, 440, form1,
                                  NULL, hInstance, NULL);
  SendMessage(lstServerListK, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  LvCol.pszText = "Server Name\0";
  ...
  SendMessage(lstServerListK, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Serverlist
  lstServerList3D = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                   listviewProperties, 10, 30, 770, 440, form1,
                                   NULL, hInstance, NULL);
  SendMessage(lstServerList3D, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  LvCol.pszText = "Server Name\0";
  ...
  SendMessage(lstServerList3D, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Login Info Frame
  tTab = CreateWindowEx(controlStyles, "SysTabControl32", NULL, tabProperties,
                        5, 478, 780, 85, form1, NULL, hInstance, NULL);
  ...
  // Login Servers
  sTab = CreateWindowEx(controlStyles, "SysTabControl32", NULL, tabProperties,
                        5, 5, 780, 470, form1, NULL, hInstance, NULL);
  ...
  if (atoi(maxUsersG) < 1 || atoi(maxUsersG) > 100)
    wsprintf(maxUsersG, "%i", 16);
  txtMaxUsers =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", maxUsersG, textboxProperties, 10,
                     520, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtMaxUsers, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  lblMaxUsers =
      CreateWindowEx(controlStyles, "STATIC", "Max Users:", labelProperties, 10,
                     505, 60, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblMaxUsers, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  if (atoi(maxPingG) < 1 || atoi(maxPingG) > 1000)
    wsprintf(maxPingG, "%i", 200);
  txtMaxPing =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", maxPingG, textboxProperties, 75,
                     520, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtMaxPing, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  lblMaxPing =
      CreateWindowEx(controlStyles, "STATIC", "Max Ping:", labelProperties, 75,
                     505, 60, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblMaxPing, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Use EmulinkerSF Options
  chkEmulinkerSF = CreateWindowEx(controlStyles, "BUTTON",
                                  "Use Extended Options?", checkProperties, 325,
                                  535, 140, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkEmulinkerSF, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkEmulinkerSF, BM_SETCHECK, emulinkerSFValue, 0);
  // Fake P2P Options
  chkFakeP2P =
      CreateWindowEx(controlStyles, "BUTTON", "Use Fake P2P?", checkProperties,
                     325, 505, 100, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkFakeP2P, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkFakeP2P, BM_SETCHECK, fakeP2PValue, 0);
  // Emulator Restriction
  chkEmuRes = CreateWindowEx(controlStyles, "BUTTON", "Restrict Emulator?",
                             checkProperties, 150, 505, 105, 25, form1, NULL,
                             hInstance, NULL);
  SendMessage(chkEmuRes, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkEmuRes, BM_SETCHECK, emuResValue, 0);
  // Connection Restriction
  chkConnRes = CreateWindowEx(controlStyles, "BUTTON",
                              "Restrict Connection Type?", checkProperties, 150,
                              535, 145, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkConnRes, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkConnRes, BM_SETCHECK, connResValue, 0);
  // Version
  btnVersion = CreateWindowEx(controlStyles, "BUTTON", "Server Version",
                              buttonProperties, 500, 530, 80, 25, form1, NULL,
                              hInstance, NULL);
  SendMessage(btnVersion, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  // Quit Textbox
  txtQuit = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", quit, textboxProperties,
                           245, 530, 205, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtQuit, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  lblQuit =
      CreateWindowEx(controlStyles, "STATIC", "Quit msg:", labelProperties, 196,
                     535, 46, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblQuit, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Stats Label
  lblStats = CreateWindowEx(controlStyles, "STATIC", cVersion, labelProperties,
                            510, 510, 270, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblStats, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Server IP Textbox
  txtServerIP =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", serverIP, textboxProperties, 40,
                     505, 150, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtServerIP, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  lblServerIP = CreateWindowEx(controlStyles, "STATIC", "IP:", labelProperties,
                               23, 510, 14, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblServerIP, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Username Textbox
  txtUsername =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", username, textboxProperties, 40,
                     530, 150, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtUsername, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  lblUsername =
      CreateWindowEx(controlStyles, "STATIC", "Nick:", labelProperties, 11, 535,
                     26, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblUsername, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Show Chatroom
  btnChatroom =
      CreateWindowEx(controlStyles, "BUTTON", "Servers", buttonProperties, 590,
                     530, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnChatroom, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Login Button
  btnLogin = CreateWindowEx(controlStyles, "BUTTON", "Login", buttonProperties,
                            655, 530, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnLogin, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Logoff Button
  btnLogoff =
      CreateWindowEx(controlStyles, "BUTTON", "Logoff", buttonProperties, 720,
                     530, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnLogoff, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Connection Type Combobox
  cmbConnectionType = CreateWindow("COMBOBOX", NULL, comboboxProperties, 495,
                                   535, 90, 500, form1, NULL, hInstance, NULL);
  SendMessage(cmbConnectionType, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  lblConnectionType =
      CreateWindowEx(controlStyles, "STATIC", "Type:", labelProperties, 462,
                     538, 30, 15, form1, NULL, hInstance, NULL);
  SendMessage(lblConnectionType, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));

  // Chatroom Options
  chkJoinChat = CreateWindowEx(controlStyles, "BUTTON",
                               "Show Join/Left Notifications?", checkProperties,
                               10, 505, 150, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkJoinChat, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkJoinChat, BM_SETCHECK, joinChatValue, 0);

  chkCreate = CreateWindowEx(
      controlStyles, "BUTTON", "Show Create/Close Notifications?",
      checkProperties, 10, 535, 180, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkCreate, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkCreate, BM_SETCHECK, createValue, 0);

  chkKeepChatLogs =
      CreateWindowEx(controlStyles, "BUTTON", "Keep Logs?", checkProperties,
                     350, 505, 100, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkKeepChatLogs, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkKeepChatLogs, BM_SETCHECK, chatLogValue, 0);

  chkShowError = CreateWindowEx(
      controlStyles, "BUTTON", "Show Rom/Emulator Errors?", checkProperties,
      180, 505, 155, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkShowError, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkShowError, BM_SETCHECK, errorValue, 0);

  chkDrop = CreateWindowEx(controlStyles, "BUTTON",
                           "Show Dropped Packet Alerts?", checkProperties, 400,
                           535, 165, 25, form1, NULL, hInstance, NULL);
  ...
  chkJoinDbl = CreateWindowEx(
      controlStyles, "BUTTON", "Allow double click to join game?",
      checkProperties, 210, 535, 170, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkJoinDbl, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkJoinDbl, BM_SETCHECK, joinDblValue, 0);

  // Gameroom Options
  chkJoinChatGame = CreateWindowEx(
      controlStyles, "BUTTON", "Show Join/Left Notifications?", checkProperties,
      10, 505, 170, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkJoinChatGame, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkJoinChatGame, BM_SETCHECK, joinChatGameValue, 0);

  chkBeep = CreateWindowEx(controlStyles, "BUTTON", "Beep when User Joins?",
                           checkProperties, 10, 535, 170, 25, form1, NULL,
                           hInstance, NULL);
  SendMessage(chkBeep, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  SendMessage(chkBeep, BM_SETCHECK, beepValue, 0);

  chkUseScreenChat = CreateWindowEx(controlStyles, "BUTTON", "Use Screen Chat?",
                                    checkProperties, 180, 535, 150, 25, form1,
                                    NULL, hInstance, NULL);
  SendMessage(chkUseScreenChat, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkUseScreenChat, BM_SETCHECK, useScreenChatValue, 0);

  chkKeepGameChatLogs = CreateWindowEx(
      controlStyles, "BUTTON", "Keep Gameroom Logs?", checkProperties, 180, 505,
      140, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkKeepGameChatLogs, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  SendMessage(chkKeepGameChatLogs, BM_SETCHECK, gameChatLogValue, 0);

  chkBlink = CreateWindowEx(controlStyles, "BUTTON", "Blink when User Joins?",
                            checkProperties, 330, 505, 140, 25, form1, NULL,
                            hInstance, NULL);
  ...
  chkUseCache = CreateWindowEx(
      controlStyles, "BUTTON", "Use Cache? [Experiment for best Performance]",
      checkProperties, 330, 535, 260, 25, form1, NULL, hInstance, NULL);
  SendMessage(chkUseCache, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  for (i = recentCount; i >= 0; i--) {
    if (/*strlen(recentServers[i].server) > 0 && */ strlen(
            recentServers[i].ip) >
        0 /* && strlen(recentServers[i].location) > 0*/)
      recentlistAdditem(recentServers[i].server, recentServers[i].ip,
                        recentServers[i].location);
  }

  // Favorites
  for (i = favoriteCount; i >= 0; i--) {
    if (/*strlen(favoriteServers[i].server) > 0 && */ strlen(
            favoriteServers[i].ip) >
        0 /* && strlen(favoriteServers[i].location) > 0*/)
      favoritelistAdditem(favoriteServers[i].server, favoriteServers[i].ip,
                          favoriteServers[i].location,
                          favoriteServers[i].comments);
  }
...
void popupAway() {
  ...
  AppendMenu(awayMenu, MF_STRING | MF_POPUP, 0xFFFF,
  ...
  for (i = 0; i <= awayMessageCount; i++) {
    if (awayMessages[i].save == true)
      AppendMenu(awayMenu, MF_STRING | MF_POPUP, i + 1,
                 awayMessages[i].subject);
  }

  // Popup Menu at Appropriate Location
  i = TrackPopupMenu(awayMenu, TPM_RETURNCMD, b.left + 725, b.top + 330, 0,
                     form1, NULL);
...
void updateAway() {
  if (awayMessageCount == (AWAY_MESSAGE_MAX - 1)) {
    MessageBox(frmAway,
               ...
               "Max Away Messages!", NULL);
    return;
  }

  awayMessageCount = awayMessageCount + 1;

  SendMessage(txtAwayMessageSubject, WM_GETTEXT, 1024,
              (LPARAM)awayMessages[awayMessageCount].subject);
  SendMessage(txtAwayMessage, WM_GETTEXT, 1024,
              (LPARAM)awayMessages[awayMessageCount].message);

  awayAdditem(awayMessages[awayMessageCount].subject,
              awayMessages[awayMessageCount].message, awayMessageCount);
...
void awayAdditem(char subject[], char message[], int value) {
  ...
  LvItem.pszText =
      subject; // Text to display (can be from a char variable) (Items)
  SendMessage(lstAway, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview

  LvItem.iSubItem = 1;
  LvItem.pszText = message;
  SendMessage(lstAway, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
...
void createEditFavoritesWindow() {
  ...
  frmEditFavorites = CreateWindow(
      "Supraclient", "Edit Favorite Entry", formOtherProperties, b.left + 110,
      b.top + 200, 330, 150, form1, NULL, hInstance, NULL);
  // IP
  txtEIP = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", strIP, textboxProperties, 5,
                          10, 250, 25, frmEditFavorites, NULL, hInstance, NULL);
  SendMessage(txtEIP, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Name
  txtEServer =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", strServer, textboxProperties, 5,
                     35, 250, 25, frmEditFavorites, NULL, hInstance, NULL);
  SendMessage(txtEServer, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Location
  txtELocation =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", strLocation, textboxProperties,
                     5, 60, 250, 25, frmEditFavorites, NULL, hInstance, NULL);
  SendMessage(txtELocation, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Comments
  txtEComments =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", strComments, textboxProperties,
                     5, 85, 250, 25, frmEditFavorites, NULL, hInstance, NULL);
  SendMessage(txtEComments, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));

  // Create New
  btnEOkay =
      CreateWindowEx(controlStyles, "BUTTON", "Okay", buttonProperties, 260, 10,
                     60, 25, frmEditFavorites, NULL, hInstance, NULL);
...
void createCommentsWindow() {
  ...
  frmComments = CreateWindow("Supraclient", "Any Comments?",
                             formOtherProperties, b.left + 110, b.top + 200,
                             330, 80, form1, NULL, hInstance, NULL);
  // Subject
  txtComments =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", NULL, textboxProperties, 5, 10,
                     250, 25, frmComments, NULL, hInstance, NULL);
  SendMessage(txtComments, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Create New
  btnComments =
      CreateWindowEx(controlStyles, "BUTTON", "Okay", buttonProperties, 260, 10,
                     60, 25, frmComments, NULL, hInstance, NULL);
  SendMessage(btnComments, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
...
void createPMWindow() {
  ...
  frmPM = CreateWindow("Supraclient", "Enter a message:", formOtherProperties,
                       b.left + 110, b.top + 200, 330, 80, form1, NULL,
                       hInstance, NULL);
  // Subject
  txtPM = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", NULL, textboxProperties, 5,
                         10, 250, 25, frmPM, NULL, hInstance, NULL);
  SendMessage(txtPM, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Create New
  btnPM = CreateWindowEx(controlStyles, "BUTTON", "Okay", buttonProperties, 260,
                         10, 60, 25, frmPM, NULL, hInstance, NULL);
...
void createAwayWindow() {
  ...
  frmAway = CreateWindow("Supraclient", "Create Away Message",
                         formOtherProperties, b.left + 110, b.top + 80, 440,
                         340, form1, NULL, hInstance, NULL);
  // Subject
  txtAwayMessageSubject = CreateWindowEx(
      WS_EX_CLIENTEDGE, "EDIT", "Enter Subject", textboxProperties, 5, 10, 250,
      25, frmAway, NULL, hInstance, NULL);
  SendMessage(txtAwayMessageSubject, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Message
  txtAwayMessage = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "Enter Message",
                                  textboxProperties, 5, 40, 250, 25, frmAway,
                                  NULL, hInstance, NULL);
  SendMessage(txtAwayMessage, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Create New
  btnCreateAway =
      CreateWindowEx(controlStyles, "BUTTON", "Create", buttonProperties, 260,
                     40, 60, 25, frmAway, NULL, hInstance, NULL);
  SendMessage(btnCreateAway, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Remove
  btnRemoveAway =
      CreateWindowEx(controlStyles, "BUTTON", "Remove", buttonProperties, 325,
                     40, 60, 25, frmAway, NULL, hInstance, NULL);
  SendMessage(btnRemoveAway, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));

  // Create Away List
  lstAway = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                           listviewProperties, 5, 80, 420, 220, frmAway, NULL,
                           hInstance, NULL);
...
void parseWaitingGames() {
  ...
  for (i = 0; i < total; i++) {
    ...
    waitinglistAdditem(game, emulator, username, server, ip, location,
                       numPlayers);
  }
...
void parseServerList3D() {
  ...
  for (i = i; i < total; i++) {
    ...
    Serverlist3DAdditem(serverName, ipAddress, "NA", location, users, games,
                        version);
  }
...
void parseServerListK() {
  ...
  for (i = i; i < total; i++) {
    ...
    // skips spam servers
    if (strstr(serverName, "kofip") || strcmp(version, "0.97") == 0 ||
        strcmp(version, "0.98") == 0 || strcmp(version, "0.99") == 0 ||
        strcmp(version, "1.00") == 0 || strcmp(version, "1.01") == 0)
      continue;
    kServerlistAdditem(serverName, ipAddress, "NA", location, users, games,
                       version);
  }
...
DWORD WINAPI pingKailleraServers(LPVOID lpParam) {
  ...
  for (i = i; i < total; i++) {
    ...
    if (sError == false) {
      if (inet_addr(strIP) == INADDR_NONE) {
        ...
        if (hp == NULL) {
          ...
          socketInfoK.sin_addr = *(struct in_addr *)(hp->h_addr_list[0]);
      } else {
    ...
    if (sError == false) {
      ...
      sendto(mySocketK, "PING\0", 5, NULL, (sockaddr *)&socketInfoK,
             sizeof(socketInfoK));
...
DWORD WINAPI ping3DServers(LPVOID lpParam) {
  ...
  for (i = i; i < total; i++) {
    ...
    if (sError == false) {
      ...
      sendto(mySocket3D, "PING\0", 5, NULL, (sockaddr *)&socketInfo3D,
             sizeof(socketInfo3D));
      ...
      while (buff < 1 && GetTickCount() - timeout <= 1000) {
        ...
        if (buff > 1) {
          if (strncmp(myBuff, "PONG", 4) == 0) {
            ...
            b.iSubItem = 2;
            b.pszText = strPing;
            SendMessage(lstServerList3D, LVM_SETITEMTEXT, (WPARAM)i,
                        (LPARAM)&b);
            here = true;
            break;
          }
...
void gameInit() {
  ...
  while (gamePlaying == false) {
    ...
    if (gameCount == 0 || sizeOfEinput == -1) {
      MessageBox(form1, "Never received ready signal. Game Ended!",
                 "gameInit()", NULL);
      sizeOfEinput = -1;
      return;
    }
...
void createChatroom() {
  ...
  txtChatroom =
      CreateWindowEx(WS_EX_CLIENTEDGE, "RichEdit", NULL, richTextboxProperties,
                     5, 5, 480, 260, form1, NULL, hInstance, NULL);
  SendMessage(txtChatroom, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  txtGameChatroom = CreateWindowEx(
      WS_EX_CLIENTEDGE, "RichEdit", "<New Session>\r\n", richTextboxProperties,
      5, 330, 480, 115, form1, NULL, hInstance, NULL);
  SendMessage(txtGameChatroom, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  ...
  SendMessage(txtGameChatroom, EM_AUTOURLDETECT, (WPARAM) true,
              0); // URL detect
  // Game Chat Textbox
  txtGameChat =
      CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", NULL, textboxProperties, 5, 450,
                     350, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtGameChat, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Game Chat Button
  btnGameChat =
      CreateWindowEx(controlStyles, "BUTTON", "Chat", buttonProperties, 360,
                     450, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnGameChat, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Lagstat Button
  btnLagStat =
      CreateWindowEx(controlStyles, "BUTTON", "Lag Stat", buttonProperties, 425,
                     450, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnLagStat, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));

  // Game Start Button
  btnGameStart =
      CreateWindowEx(controlStyles, "BUTTON", "Start", buttonProperties, 490,
                     300, 50, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnGameStart, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Game Kick Button
  btnGameKick =
      CreateWindowEx(controlStyles, "BUTTON", "Kick", buttonProperties, 550,
                     300, 50, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnGameKick, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Game Drop Button
  btnDrop = CreateWindowEx(controlStyles, "BUTTON", "Drop", buttonProperties,
                           610, 300, 50, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnDrop, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Game Away Button
  btnAway = CreateWindowEx(controlStyles, "BUTTON", "Away", buttonProperties,
                           670, 300, 50, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnAway, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Game Leave Button
  btnGameLeave =
      CreateWindowEx(controlStyles, "BUTTON", "Leave", buttonProperties, 730,
                     300, 50, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnGameLeave, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));

  // Game Name Textbox
  txtGame = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "Not in Game!",
                           buttonProperties | ES_READONLY | WS_BORDER, 5, 300,
                           480, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtGame, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Create GameUserlist
  lstGameUserlist = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                                   listviewProperties, 490, 330, 295, 145,
                                   form1, NULL, hInstance, NULL);
  SendMessage(lstGameUserlist, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  ...
  SendMessage(lstGameUserlist, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Userlist
  lstUserlist = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                               listviewProperties, 490, 5, 295, 290, form1,
                               NULL, hInstance, NULL);
  SendMessage(lstUserlist, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  LvCol.pszText = "UserID\0";
  ...
  SendMessage(lstUserlist, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Create Gamelist
  lstGamelist = CreateWindowEx(WS_EX_CLIENTEDGE, "SysListView32", NULL,
                               listviewProperties, 5, 300, 780, 175, form1,
                               NULL, hInstance, NULL);
  SendMessage(lstGamelist, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  memset(&LvCol, 0, sizeof(LvCol));
  LvCol.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
  ...
  SendMessage(lstGamelist, LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
              (LPARAM)listviewStyles);
  // Chat Textbox
  txtChat = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", NULL, textboxProperties, 5,
                           270, 285, 25, form1, NULL, hInstance, NULL);
  SendMessage(txtChat, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Chat Button
  btnChat = CreateWindowEx(controlStyles, "BUTTON", "Chat", buttonProperties,
                           295, 270, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnChat, WM_SETFONT, (WPARAM)hDefaultFont, MAKELPARAM(FALSE, 0));
  // Create Button
  btnCreate =
      CreateWindowEx(controlStyles, "BUTTON", "Create", buttonProperties, 360,
                     270, 60, 25, form1, NULL, hInstance, NULL);
  SendMessage(btnCreate, WM_SETFONT, (WPARAM)hDefaultFont,
              MAKELPARAM(FALSE, 0));
  // Join Button
  btnJoin = CreateWindowEx(controlStyles, "BUTTON", "Join", buttonProperties,
                           425, 270, 60, 25, form1, NULL, hInstance, NULL);
...
void fixLastGameToPlay() {
  if (strcmp(lastGameToPlay1, "LastGameToPlay1") == 0) {
    ...
    ModifyMenu(gameMenu, (UINT)0xFFFD, MF_BYCOMMAND, (UINT)0xFFFD,
               (LPCSTR)lastGameToPlay1);
    ...
    ModifyMenu(gameMenu, (UINT)0xFFFD, MF_BYCOMMAND, (UINT)0xFFFD,
               (LPCSTR)lastGameToPlay1);
    ModifyMenu(gameMenu, (UINT)0xFFFE, MF_BYCOMMAND, (UINT)0xFFFE,
               (LPCSTR)lastGameToPlay2);
    ...
    ModifyMenu(gameMenu, (UINT)0xFFFE, MF_BYCOMMAND, (UINT)0xFFFE,
               (LPCSTR)lastGameToPlay2);
    ModifyMenu(gameMenu, (UINT)0xFFFF, MF_BYCOMMAND, (UINT)0xFFFF,
               (LPCSTR)lastGameToPlay3);
  } else {
    if (strcmp(lastGameToPlay1, currentGame) == 0)
      return;
    else if (strcmp(lastGameToPlay2, currentGame) == 0) {
...
void popupMenu(char num) {
  ...
  if (num == 0) {
    // Popup Menu at Appropriate Location
    i = TrackPopupMenu(gameMenu, TPM_RETURNCMD, b.left + 425, b.top + 300, 0,
                       form1, NULL);
  ...
  // Game Userlist
    ...
    // Popup Menu at Appropriate Location
    GetCursorPos(&c);
    i = TrackPopupMenu(gameUserlistMenu, TPM_RETURNCMD, c.x, c.y, 0, form1,
                       NULL);
    ...
    // Ping
      ...
      if (tempS == lstServerListK) {
        if (pingingK == false) {
          ...
          kailleraPingThread =
              CreateThread(NULL, 0, pingKailleraServers, NULL, 0, NULL);
        }
      } else {
        if (pinging3D == false) {
          exitPing3DThread();
  ...
  // Waiting Games Menu
    ...
    // Popup Menu at Appropriate Location
    GetCursorPos(&c);
    i = TrackPopupMenu(waitinglistMenu, TPM_RETURNCMD, c.x, c.y, 0, form1,
                       NULL);
  ...
  // Favorite List Menu
    ...
    // Popup Menu at Appropriate Location
    GetCursorPos(&c);
    i = TrackPopupMenu(favoritelistMenu, TPM_RETURNCMD, c.x, c.y, 0, form1,
                       NULL);
  ...
  // txtChatroom
    ...
    // Popup Menu at Appropriate Location
    GetCursorPos(&c);
    i = TrackPopupMenu(txtChatroomMenu, TPM_RETURNCMD, c.x, c.y, 0, form1,
                       NULL);
...
int saveConfig() {
  ...
  if (!config.good()) {
    MessageBox(form1, "Error occurred while saving to supraclient.ini",
               "Error Saving!", NULL);
    return 1;
  }
  ...
  // Connection
  GetWindowText(cmbConnectionType, temp,
                GetWindowTextLength(cmbConnectionType) + 1);
...
void loginToServer() {
  ...
  sendto(mySocket, entryMsg, 10, NULL, (sockaddr *)&socketInfo,
         sizeof(socketInfo));
}
...
DWORD WINAPI recvLoop(LPVOID lpParam) {
  ...
  for (;;) {
    ...
    mainBytesRecv = recv(mySocket, myBuff[myBuffCount].myBuff,
                         sizeof(myBuff[myBuffCount].myBuff), NULL);
    ...
    if (mainBytesRecv > 0) {
      ...
      if (msgType >= 0x01 && msgType <= 0x17) {
        ...
        if (lenUsername > 0) {
          ...
          MessageBox(form1, "Please supply a username!",
                     "Error UserLoginInformation", NULL);
  ...
  // Recover
    ...
    if (msgNumCount > numOfMessages) {
      if (SendMessage(chkDrop, BM_GETCHECK, 0, 0) == BST_CHECKED)
        displayAndAutoScrollRichEdit(txtChatroom,
                                     "<ALERT> PACKET(S) DROPPED. Desynch may "
                                     "occur or server updates may be missed.\n",
                                     RGB(255, 0, 0));
    ...
    for (i = 0; i < msgNumCount; i++) {
      ...
      if (i != 0)
        serverMessage[i].msgPos =
            4 + serverMessage[i - 1].msgLen + serverMessage[i - 1].msgPos;
      temp += serverMessage[i].msgLen; // includes msgType
    }

    for (i = (msgNumCount - 1); i >= 0; i--) {
      if (i != 0)
        gotoMessageType(6 + serverMessage[i].msgPos, serverMessage[i].msgType,
                        slot);
      else
        gotoMessageType(6, msgType, slot);
    }
...
void constructPacket(char *data, unsigned short lenData, char msgType) {
  ...
  if (msgType != 0) {
    ...
    sendto(mySocket, globalPacket, normalPacketSize, NULL,
           (sockaddr *)&socketInfo, sizeof(socketInfo));
...
bool supraCleanup(char type, HWND h) {
  ...
  if (type == 0) {
    ...
    // Display
    // SendMessage(form1, WM_SETTEXT, 0, (LPARAM) "SupraclientCPPE
    // https://god-weapon.github.io");
...
void kServerlistAdditem(char *serverName, char *ip, char *ping, char *location,
  ...
  LvItem.iSubItem = 0;      // Put in first coluom
  LvItem.pszText =
      serverName; // Text to display (can be from a char variable) (Items)
  SendMessage(lstServerListK, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview
  LvItem.iSubItem = 1;
  LvItem.pszText = ip;
  SendMessage(lstServerListK, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
  LvItem.pszText = ping;
  SendMessage(lstServerListK, LVM_SETITEM, 0,
...
void Serverlist3DAdditem(char *serverName, char *ip, char *ping, char *location,
  ...
  LvItem.pszText =
      serverName; // Text to display (can be from a char variable) (Items)
  SendMessage(lstServerList3D, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview
  LvItem.iSubItem = 1;
  LvItem.pszText = ip;
  SendMessage(lstServerList3D, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
  LvItem.pszText = ping;
  SendMessage(lstServerList3D, LVM_SETITEM, 0,
...
void userlistAdditem(char *nick, char *ping, char *connection, char *userID,
  ...
  LvItem.iSubItem = 0;      // Put in first coluom
  LvItem.pszText =
      userID; // Text to display (can be from a char variable) (Items)
  SendMessage(lstUserlist, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview

  LvItem.iSubItem = 1;
  LvItem.pszText = nick;
  SendMessage(lstUserlist, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
  LvItem.pszText = ping;
  SendMessage(lstUserlist, LVM_SETITEM, 0,
...
void recentlistAdditem(char *server, char *ip, char *location) {
  ...
  LvItem.iSubItem = 0;
  LvItem.pszText = ip;
  SendMessage(lstRecentList, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 1;          // Put in first coluom
  LvItem.pszText =
      tempServer; // Text to display (can be from a char variable) (Items)
  SendMessage(lstRecentList, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview
  LvItem.iSubItem = 2;
...
void waitinglistAdditem(char *game, char *emulator, char *username,
  ...
  LvItem.iSubItem = 0;      // Put in first coluom
  LvItem.pszText =
      game; // Text to display (can be from a char variable) (Items)
  SendMessage(lstWaitingList, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview
  LvItem.iSubItem = 1;
  LvItem.pszText = emulator;
  SendMessage(lstWaitingList, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
...
void favoritelistAdditem(char *server, char *ip, char *location,
  ...
  SendMessage(lstFavoriteList, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem); // Send info to the Listview
  LvItem.iSubItem = 1;
  LvItem.pszText = server;
  SendMessage(lstFavoriteList, LVM_SETITEM, 0,
              (LPARAM)&LvItem); // Enter text to SubItems
  LvItem.iSubItem = 2;
  LvItem.pszText = location;
  SendMessage(lstFavoriteList, LVM_SETITEM, 0,
...
void gameUserlistAdditem(char nick[], char ping[], char connection[],
                         char userID[]) {
  LVITEM LvItem2;
  ...
  LvItem2.pszText =
      userID; // Text to display (can be from a char variable) (Items)
  SendMessage(lstGameUserlist, LVM_INSERTITEM, 0,
              (LPARAM)&LvItem2); // Send info to the Listview
  LvItem2.iSubItem = 1;
  LvItem2.pszText = nick;
  SendMessage(lstGameUserlist, LVM_SETITEM, 0,
              (LPARAM)&LvItem2); // Enter text to SubItems
  LvItem2.iSubItem = 2;
...
void gamelistAdditemAll() {
  ...
  for (w = 0; w < tempGameCount; w++) {
    ...
    LvItem1.pszText =
        tempGames[w]
            .gameID; // Text to display (can be from a char variable) (Items)
    SendMessage(lstGamelist, LVM_INSERTITEM, 0,
                (LPARAM)&LvItem1); // Send info to the Listview
    LvItem1.iSubItem = 1;
    LvItem1.pszText = tempGames[w].game;
    SendMessage(lstGamelist, LVM_SETITEM, 0,
                (LPARAM)&LvItem1); // Enter text to SubItems
    LvItem1.iSubItem = 2;
...
void gameChatNotification(unsigned short position, int slot) {
  ...
  if (nick[0] == 'S' && nick[1] == 'e' && nick[2] == 'r' && nick[3] == 'v' &&
      nick[4] == 'e' && nick[5] == 'r' && nick[6] == '\0') {
...
void gameChatRequest() {
  ...
  int lenChat =
      SendMessage(txtGameChat, WM_GETTEXT, 255, (LPARAM)&dataToBeSent[1]);
...
void quitGameNotification(unsigned short position, int slot) {
  ...
  if (userID == myUserID) {
    if (iQuit == false)
      displayAndAutoScrollRichEdit(
          txtChatroom, "<ALERT> YOU HAVE BEEN KICKED FROM THE GAME!\n",
          RGB(255, 0, 0));
    iQuit = false;
    supraCleanup(1, 0);
    showGameroom(false);
...
void joinGameRequest() {
  ...
  for (i = 0; i < totalGames; i++) {
    if (strcmp(gameList[i].game, currentGame) == 0)
      ...
      w = MessageBox(form1, "Don't have Game!  Do you still want to join?",
                     "Game Not Available!", MB_YESNO);
      if (w == IDYES)
        break;
      else
        return;
  ...
  if (strcmp(strEmu, emulator) != 0) {
    w = MessageBox(form1, "Different Emulators!  Do you still want to join?",
                   "Emulator Not Compatible!", MB_YESNO);
    if (w == IDNO)
      return;
  }
  ...
  if (strcmp(strStatus, "Playing") == 0) {
    w = MessageBox(form1, "Currently Playing!  Do you still want to join?",
                   "Game is Playing!", MB_YESNO);
    if (w == IDNO)
      return;
  }
  //}
...
void gameDataRecv(unsigned short position, int slot) {
  ...
  if (inCachePos >= cacheSize) {
    ...
    for (i = 0; i < cacheSize; i++) {
      memcpy(&inCache[i * inCacheSize], &inCache[((i + 1) * inCacheSize)],
             totalInput);
    }
    memcpy(&inCache[lastInCachePos], &myBuff[slot].myBuff[position + 3],
           totalInput);
  }
  // Cache Values
  else {
    memcpy(&inCache[inCachePos * inCacheSize],
           &myBuff[slot].myBuff[position + 3], totalInput);
    inCachePos++;
  }
}
...
void gameDataSend() {
  ...
  if (useCache == true) {
    ...
    if (outCachePos >= cacheSize) {
      for (i = 0; i < cacheSize; i++)
        memcpy(&outCache[i * outCacheSize], &outCache[(i + 1) * outCacheSize],
               sizeOfVinput);
...
void gameCacheRecv(unsigned short position, int slot) {
  ...
  for (i = 0; i < totalInput; i++) {
    eInput[ePos] =
        inCache[(*((unsigned char *)&myBuff[slot].myBuff[position + 1]) *
                 inCacheSize) +
                i];
    ePos = (ePos + 1) % eSize;
  }
...
int CALLBACK lstUserlistCompareFunc(LPARAM i1, LPARAM i2, LPARAM) {
  ...
  if (lstUserlistColumn == 0 || lstUserlistColumn == 2) {
    ...
    if (userlistSwitch == false)
      ...
      return (n2 - n1);
  } else if (lstUserlistColumn == 1 || lstUserlistColumn == 3 ||
             lstUserlistColumn == 4) {
    if (userlistSwitch == false) {
      return _tcscmp(strToLower(tsz1), strToLower(tsz2));
    } else {
...
int CALLBACK lstServerlistKCompareFunc(LPARAM i1, LPARAM i2, LPARAM) {
  ...
  if (lstServerlistKColumn == 0 || lstServerlistKColumn == 1 ||
      lstServerlistKColumn == 3 || lstServerlistKColumn == 6) {
    if (kailleraSwitch == false) {
      return _tcscmp(strToLower(tsz1), strToLower(tsz2));
    } else {
      return _tcscmp(strToLower(tsz2), strToLower(tsz1));
    }
  } else if (lstServerlistKColumn == 2 || lstServerlistKColumn == 4 ||
             lstServerlistKColumn == 5) {
    if (strstr(tsz1, ">1s"))
      n1 = 1000;
    else if (strstr(tsz1, "ERR"))
      n1 = 1001;
    else if (strstr(tsz1, "NA"))
      n1 = 1002;
    else
      n1 = _tcstol(tsz1, 0, 10);
    ...
    if (strstr(tsz2, ">1s"))
      n2 = 1000;
    else if (strstr(tsz2, "ERR"))
      n2 = 1001;
    else if (strstr(tsz2, "NA"))
      n2 = 1002;
    else
      n2 = _tcstol(tsz2, 0, 10);
...
int CALLBACK lstServerlist3DCompareFunc(LPARAM i1, LPARAM i2, LPARAM) {
  ...
  if (lstServerlist3DColumn == 0 || lstServerlist3DColumn == 1 ||
      lstServerlist3DColumn == 3 || lstServerlist3DColumn == 6) {
    if (anti3DSwitch == false) {
      return _tcscmp(strToLower(tsz1), strToLower(tsz2));
    } else {
      return _tcscmp(strToLower(tsz2), strToLower(tsz1));
    }
  } else if (lstServerlist3DColumn == 2 || lstServerlist3DColumn == 4 ||
             lstServerlist3DColumn == 5) {
    if (strstr(tsz1, ">1s"))
      n1 = 1000;
    else if (strstr(tsz1, "ERR"))
      n1 = 1001;
...
void getServerList3D() {
  ...
  if (inet_addr(anti3DServerList.host) == INADDR_NONE) {
    ...
    if (remoteHost == NULL) {
      closesocket(mySocket3D);
      MessageBox(form1, "Could not resolve Server List!  Server may be down",
                 "Error Resolving Address!", NULL);
      return;
    } else
  ...
  WSAAsyncSelect(mySocket3D, form1, SC_SUPRARECV3D,
                 FD_WRITE | FD_CONNECT | FD_READ | FD_CLOSE);
  x = connect(mySocket3D, (sockaddr *)&socketInfo3D, sizeof(socketInfo3D));
  SetWindowText(form1, "Sending GET Request...If a list doesn't arrive within "
                       "a few seconds, the host is probably down!");
}
...
void getWaitingGames() {
  ...
  if (inet_addr(anti3DServerList.host) == INADDR_NONE) {
    ...
    if (remoteHost == NULL) {
      ...
      socketInfoWaiting.sin_addr.s_addr =
          *((unsigned long *)remoteHost->h_addr);
  } else {
    socketInfoWaiting.sin_addr.s_addr = inet_addr(anti3DServerList.host);
  }

  WSAAsyncSelect(mySocketWaiting, form1, SC_SUPRARECVWAITING,
                 FD_WRITE | FD_CONNECT | FD_READ | FD_CLOSE);
  x = connect(mySocketWaiting, (sockaddr *)&socketInfoWaiting,
              sizeof(socketInfoWaiting));
  SetWindowText(form1, "Sending GET Request...If a list doesn't arrive within "
                       "a few seconds, the host is probably down!");
}
...
void getServerListK() {
  ...
  WSAAsyncSelect(mySocketK, form1, SC_SUPRARECVK,
                 FD_WRITE | FD_CONNECT | FD_READ | FD_CLOSE);
